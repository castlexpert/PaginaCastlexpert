...
export default function CookiePolicyModal({ open, onClose, content }: CookiePolicyModalProps) {
  useEffect(() => {
    if (!open) return;
    ...
    <div className="fixed inset-0 z-[92]">
      ...
      <div className="relative mx-auto flex h-[100dvh] max-w-3xl items-end px-3 pb-3 sm:items-center sm:px-6 sm:pb-0">
        <div
          role="dialog"
          aria-modal="true"
          aria-label={content.policyTitle}
          className="relative w-full max-h-[90dvh] overflow-hidden rounded-3xl cx-card shadow-2xl shadow-black/30"
        >
          <div className="sticky top-0 z-10 border-b border-white/10 bg-white/10 backdrop-blur-2xl">
            <div className="flex items-center justify-between gap-4 px-5 py-4 sm:px-8 sm:py-5">
              <h3 className="text-2xl font-black text-zinc-950 sm:text-3xl">{content.policyTitle}</h3>
              <button
                type="button"
                onClick={onClose}
          ...
          <div className="space-y-4 overflow-y-auto px-5 py-6 text-sm leading-relaxed text-zinc-800 sm:px-8 sm:text-base">
            {content.policyParagraphs.map((paragraph) => (
